...
class BoardApplicantIntake
{
    ...
    public static function batchDuplicateRowIDs($rows)
    {
        ...
        foreach ((array) $rows as $row)
        {
            ...
            $nameKey = strtolower(trim($row['first_name'])) . "\0" . strtolower(trim($row['last_name']));
            if (count($emailRows[$emailKey]) > 1 || count($nameRows[$nameKey]) > 1)
            {
    ...
    public static function parseCsv($contents, $platform, $jobOrderID, $sourceLabel)
    {
        ...
        foreach (array('first_name', 'last_name', 'email') as $required)
        {
        ...
        try
        {
            $sql = sprintf(
                'INSERT INTO nesp_board_intake_batch
                    (platform_key, joborder_id, source_label, source_checksum, status_key, row_count, created_by_user_id, date_created, date_modified)
                 VALUES (%s, %s, %s, %s, "review", %s, %s, NOW(), NOW())',
    ...
    public function approveRows($batchID, $rowIDs)
    {
    ...
    public function importApprovedRows($actorUserID, $batchID)
    {
        ...
        {
            throw new RuntimeException('Batch is not awaiting review.');
        }
        ...
        try
        {
            foreach ($rows as $row)
            {
                ...
                if ($row['external_id'] !== null && $this->findImportedIdempotencyCandidate($row['platform_key'], $row['external_id']))
                {
                ...
                $sql = sprintf(
                    'UPDATE nesp_board_intake_row
                        SET review_status = "imported", candidate_id = %s, date_modified = NOW()
