...
<div id="main">
    ...
    <div id="contents">
        ...
        <form action="<?php echo CATSUtility::getIndexName(); ?>?m=boardintake&amp;a=upload" method="post" enctype="multipart/form-data">
            ...
            <p>Accepted columns: external_id, first_name, last_name, email, phone. Resume and attachment URLs are not accepted.</p>
        ...
        <h3>2. Review rows</h3>
        <p>Only valid rows with duplicate status <strong>none</strong> may be approved.</p>
        <form action="<?php echo CATSUtility::getIndexName(); ?>?m=boardintake&amp;a=approve" method="post">
            ...
            <table class="searchTable" width="100%">
                ...
                <tr>
                    <td><?php if ($row['validation_status'] === 'valid' && $row['duplicate_status'] === 'none'): ?><input type="checkbox" name="approvedRows[]" value="<?php echo (int) $row['intake_row_id']; ?>" <?php if ($row['review_status'] === 'approved'): ?>checked<?php endif; ?> /><?php endif; ?></td>
        ...
        <p>This is the only action that creates candidates and attaches them to job order <?php echo (int) $this->batch['joborder_id']; ?>. It is transactional and does not send messages.</p>
