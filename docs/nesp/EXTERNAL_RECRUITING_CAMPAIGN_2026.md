# NESP External Recruiting Campaign - Four Approved Roles

Prepared for the first public recruiting campaign using
`https://careers.nesportsphoto.com` as the application destination.

Craig has approved including the Freelance/Contract Youth Sports Photographer
role in this campaign. Facebook is intentionally excluded from this campaign.
Do not ask applicants to email resumes; send candidates to the matching NESP
application link. The NESP application form is the single applicant workflow
for this campaign and now includes role-specific prescreening questions.

## Approved Roles And Direct Links

| Job ID | Role | Pay | Classification | Location / Territory | Application link |
| --- | --- | --- | --- | --- | --- |
| `41001` | Part-Time Customer Service Representative | `$22-$25/hour` | Part-time seasonal W-2 | Methuen, MA | `https://careers.nesportsphoto.com/careers/?p=applyToJob&ID=41001` |
| `41002` | Weekend Staff Portrait & Team Photographer - Youth Sports | `$22-$25/hour` | Part-time temporary seasonal W-2 | MA, CT, RI, VT, NH | `https://careers.nesportsphoto.com/careers/?p=applyToJob&ID=41002` |
| `41003` | Freelance/Contract Youth Sports Photographer | `$22-$27/hour` | Freelance/Independent Contractor | MA, RI, NH, CT | `https://careers.nesportsphoto.com/careers/?p=applyToJob&ID=41003` |
| `41005` | Weekend Table Greeter / Field Assistant | `$18/hour` | Part-time temporary seasonal W-2 | Primarily MA, with CT, RI, NH assignments | `https://careers.nesportsphoto.com/careers/?p=applyToJob&ID=41005` |

Important distinction:

- Staff photographers are W-2 employees. NESP provides equipment and training.
- Freelance/contract photographers are independent contractors. They provide
  their own approved professional equipment.
- Field assistants are W-2 employees. NESP provides training, workflow
  guidance, and posing guides.

## Platform Cost Notes

- Indeed: free standard publication is approved. Stop before sponsorship,
  matched-candidate purchase, billing setup, or paid promotion unless Craig
  approves the specific role, price, duration, and total.
- Craigslist: Craig approved up to `$180` total. The Boston jobs flow previously
  showed `$45` per selected job category, so prepare four separate compliant
  `$45` ads and stop before payment confirmation unless Craig is present.
- College career boards, local job boards, and photography communities: use
  free or low-cost employer postings first. Stop before account creation,
  payment, identity verification, or final publication unless Craig approves.

## Indeed Posting Package

### 41001 - Customer Service

- Platform-ready title: `Part-Time Customer Service Representative - Methuen, MA`
- Opening: `New England Sports Photo is hiring a part-time Customer Service Representative for our Methuen office. This role helps parents, families, league coordinators, and school contacts with online ordering, delivery, account, reprint, and support questions.`
- Pay: `$22-$25/hour, based on experience`
- Location: `Methuen, MA`
- Schedule: `Weekday daytime schedule by agreement, generally 9:00 AM-3:00 PM or 10:00 AM-4:00 PM during spring and fall peak seasons`
- Employment type: `Part-time seasonal W-2`
- Responsibilities:
  - Respond to customer emails, support tickets, and calls
  - Help parents with online ordering and account questions
  - Research shipping, delivery, order-status, reprint, and replacement requests
  - Support league and school coordinators
  - Escalate complex or sensitive issues to Craig
- Qualifications:
  - Customer-service or administrative experience preferred
  - Strong written and verbal communication
  - Comfortable with email, spreadsheets, support systems, and web applications
  - Organized, detail-oriented, and dependable
- Application method/link: external NESP application,
  `https://careers.nesportsphoto.com/careers/?p=applyToJob&ID=41001`
- Suggested category: `Customer Service`
- Suggested location: `Methuen, MA`
- Screening questions:
  - `Are you able to work in person in Methuen, Massachusetts?`
  - `Are you generally available Monday-Friday during the stated daytime schedule?`
  - `How many years of customer-service or administrative experience do you have?`
- Image/logo: approved NESP logo only; do not use athlete photos unless cleared.

### 41002 - Staff Photographer

- Platform-ready title: `Weekend Staff Portrait & Team Photographer - Youth Sports`
- Opening: `New England Sports Photo is hiring seasonal W-2 staff photographers for youth-sports Picture Day events. NESP provides the camera and lighting equipment, paid training, and a structured Picture Day workflow.`
- Pay: `$22-$25/hour, based on experience`
- Territory: `MA, CT, RI, VT, NH`
- Schedule: `Most assignments are Saturday mornings, with some Sundays and optional weekday after-school assignments`
- Employment type: `Part-time temporary seasonal W-2`
- Responsibilities:
  - Photograph individual athletes and teams using NESP equipment
  - Set up cameras, lighting, and related gear
  - Work professionally with athletes, parents, coaches, and league staff
  - Follow the NESP Picture Day workflow
- Qualifications:
  - Reliable transportation and valid driver's license
  - Early-morning weekend availability
  - Comfort working with children and families
  - Photography, youth-sports, school, event, or customer-facing experience helpful
  - Ability to pass required background checks and youth-sports screenings
- Application method/link: external NESP application,
  `https://careers.nesportsphoto.com/careers/?p=applyToJob&ID=41002`
- Suggested category: `Photography`, `Events`, or `Seasonal`
- Suggested location: `Methuen, MA` or `Massachusetts`
- Screening questions:
  - `Do you have reliable transportation and a valid driver's license?`
  - `Are you available for early-morning weekend assignments during the spring season?`
  - `Are you comfortable working with children and families?`
  - `Are you willing to travel to assigned youth-sports locations?`
- Image/logo: approved NESP logo or cleared NESP Picture Day setup image.

### 41003 - Freelance Photographer

- Platform-ready title: `Freelance Youth Sports Photographer - Picture Day Assignments`
- Opening: `New England Sports Photo is seeking freelance/contract photographers for youth-sports Picture Day assignments. This contractor role is for photographers who provide their own approved professional equipment and are comfortable following a structured volume-photography workflow.`
- Pay: `$22-$27/hour, based on experience`
- Territory: `MA, RI, NH, CT`
- Schedule: `Primarily early-morning weekend assignments; optional after-school weekday work may be available`
- Employment type: `Freelance/Independent Contractor`
- Responsibilities:
  - Photograph youth athletes and teams at assigned Picture Day events
  - Bring approved professional camera, lens, external flash, and backup equipment
  - Follow NESP's Picture Day process and event instructions
  - Work professionally with athletes, parents, coaches, and league staff
- Qualifications:
  - Own approved professional camera body, external speedlight, and suitable portrait zoom lens
  - Comfort using manual camera and flash settings
  - Reliable transportation and weekend availability
  - Portrait, event, sports, school, or volume-photography experience preferred
- Application method/link: external NESP application,
  `https://careers.nesportsphoto.com/careers/?p=applyToJob&ID=41003`
- Suggested category: `Photography`, `Events`, or `Contract`
- Suggested location: `Massachusetts`, with territory noted in the description
- Screening questions:
  - `Do you own an approved professional camera, external speedlight, and suitable portrait zoom lens?`
  - `Are you comfortable using manual camera and flash settings?`
  - `Are you available for early-morning weekend assignments?`
  - `Are you willing to travel approximately 60-90 minutes to assignments?`
  - `Briefly describe your portrait, event, or sports-photography experience.`
- Image/logo: approved NESP logo. Do not describe employee benefits as
  contractor benefits.

### 41005 - Field Assistant

- Platform-ready title: `Weekend Table Greeter / Field Assistant - Youth Sports`
- Opening: `New England Sports Photo is hiring friendly weekend field assistants to help youth-sports Picture Day events run smoothly. This active role welcomes families, organizes teams, checks forms, and supports the photographer.`
- Pay: `$18/hour`
- Territory: `Primarily MA, with CT, RI, and NH assignments`
- Schedule: `Primarily Saturday mornings, with some Sundays during the spring season`
- Employment type: `Part-time temporary seasonal W-2`
- Responsibilities:
  - Welcome and check in families
  - Organize teams and help manage Picture Day flow
  - Review order forms for completeness and accuracy
  - Help keep portrait and team sessions on schedule
  - Support photographers in the field
- Qualifications:
  - Friendly, professional communication
  - Organized and detail-oriented
  - Reliable transportation and valid driver's license
  - Comfortable standing, working outdoors, and lifting up to 25 pounds
  - Ability to pass required background checks
- Application method/link: external NESP application,
  `https://careers.nesportsphoto.com/careers/?p=applyToJob&ID=41005`
- Suggested category: `Event Staff`, `Customer Service`, or `Seasonal`
- Suggested location: `Massachusetts`
- Screening questions:
  - `Do you have reliable transportation and a valid driver's license?`
  - `Are you available for early-morning weekend assignments?`
  - `Are you comfortable working outdoors and directing groups?`
  - `Can you stand for extended periods and lift up to 25 pounds?`
- Image/logo: approved NESP logo.

## NESP Prescreening Questions

Each direct NESP application form asks job-related prescreening questions before
CAPTCHA and submission. The form shows this statement above the questions:

`These questions help the NESP hiring team understand your availability and job-related qualifications. Every application is reviewed by a person.`

Answers are stored in OpenCATS candidate questionnaire history as
`NESP Prescreen - [role title]` and are visible from the candidate profile. The
answers do not automatically reject, rank, hire, assign, email, or change status.

Do not duplicate these questions in Indeed, Craigslist, Handshake, or other
platforms when candidates are being directed to the NESP application link.

## Craigslist Posting Plan

Budget cap: `$180`.

Observed Boston jobs fee: approximately `$45` per selected job posting.

Use four separate postings because Craigslist permits one job description per
posting:

1. Customer Service Representative in `boston > northwest/merrimack > jobs >
   customer service`.
2. Staff Photographer in the most appropriate regional jobs or gigs category,
   such as `art/media/design`, `tv/film/video/radio`, or a photography/event
   gigs category if Craigslist offers it.
3. Freelance/Contract Youth Sports Photographer in the closest available
   photography, creative, or event category.
4. Weekend Table Greeter / Field Assistant in the closest available event staff,
   customer service, seasonal, or field support category.

### Craigslist Post 1 - Customer Service

- Title: `Part-Time Customer Service Representative - Methuen - $22-$25/hr`
- Region: `boston > northwest/merrimack`
- Category: `jobs > customer service`
- Cost: expected `$45`
- Body:

```text
New England Sports Photo is hiring a part-time Customer Service Representative for our Methuen office.

Pay: $22-$25 per hour, based on experience
Location: Methuen, MA - in office
Employment type: part-time seasonal W-2
Schedule: weekday daytime hours during spring and fall peak seasons

What you'll do:
- Respond to customer emails, support tickets, and calls
- Help parents with online ordering and account questions
- Research shipping, delivery, order-status, reprint, and replacement requests
- Support league and school coordinators
- Escalate complex or sensitive issues to Craig

What we're looking for:
- Strong written and verbal communication
- Good computer skills
- Organized, detail-oriented work habits
- Ability to multitask during busy seasons
- Customer-service or administrative experience preferred

Apply through NESP:
https://careers.nesportsphoto.com/careers/?p=applyToJob&ID=41001

Applications are reviewed by the New England Sports Photo hiring team.
```

### Craigslist Post 2 - Staff Photographer

- Title: `Weekend Youth Sports Staff Photographer - $22-$25/hr - Equipment Provided`
- Region: first choice `boston > northwest/merrimack`; additional regions only
  if budget allows and rules permit
- Category: `jobs > art/media/design`, `jobs > tv/film/video/radio`, or the
  closest available photography/event category
- Cost: expected `$45`
- Body:

```text
New England Sports Photo is hiring seasonal W-2 staff photographers for youth-sports Picture Day events.

Pay: $22-$25 per hour, based on experience
Territory: MA, CT, RI, VT, and NH
Employment type: part-time temporary seasonal W-2
Schedule: most Saturday mornings, some Sundays, optional weekday after-school assignments
Equipment: NESP provides camera equipment, lighting, training, and workflow guidance

What you'll do:
- Photograph individual athletes and teams using NESP equipment
- Set up cameras, lighting, and related gear
- Work professionally with athletes, parents, coaches, and league staff
- Follow the NESP Picture Day workflow

What we're looking for:
- Reliable transportation and valid driver's license
- Early-morning weekend availability
- Comfort working with children and families
- Photography, youth-sports, school, event, or customer-facing experience helpful
- Ability to pass required background checks and youth-sports screenings

Apply through NESP:
https://careers.nesportsphoto.com/careers/?p=applyToJob&ID=41002

Applications are reviewed by the New England Sports Photo hiring team.
```

### Craigslist Post 3 - Freelance Photographer

- Title: `Freelance Youth Sports Photographer - $22-$27/hr`
- Region: first choice `boston > northwest/merrimack`; choose another region
  only if Craig approves it and budget remains
- Category: closest available photography, creative, event, or contract category
- Cost: expected `$45`
- Body:

```text
New England Sports Photo is seeking freelance/contract photographers for youth-sports Picture Day assignments.

Pay: $22-$27 per hour, based on experience
Classification: Freelance/Independent Contractor
Territory: MA, RI, NH, and CT
Schedule: primarily early-morning weekend Picture Day assignments
Equipment: freelance photographers provide their own approved professional camera body, external speedlight, suitable portrait zoom lens, and backup equipment

What you'll do:
- Photograph youth athletes and teams at assigned Picture Day events
- Bring approved professional equipment
- Follow NESP's Picture Day workflow and event instructions
- Work professionally with athletes, families, coaches, and league staff

What we're looking for:
- Own a professional or advanced camera body suitable for portrait work
- Own an external speedlight and suitable portrait zoom lens
- Comfort using manual camera and flash settings
- Reliable transportation and early-morning weekend availability

Apply through NESP:
https://careers.nesportsphoto.com/careers/?p=applyToJob&ID=41003

Applications are reviewed by the New England Sports Photo hiring team.
```

### Craigslist Post 4 - Field Assistant

- Title: `Weekend Table Greeter / Field Assistant - Youth Sports - $18/hr`
- Region: first choice `boston > northwest/merrimack`; choose another region
  only if Craig approves it and budget remains
- Category: closest available event staff, customer service, seasonal, or field
  support category
- Cost: expected `$45`
- Body:

```text
New England Sports Photo is hiring friendly weekend field assistants to help youth-sports Picture Day events run smoothly.

Pay: $18 per hour
Employment type: part-time temporary seasonal W-2
Territory: primarily MA, with CT, RI, and NH assignments
Schedule: primarily Saturday mornings, with some Sundays
Equipment/training: NESP provides training, Picture Day workflow guidance, and posing guides

What you'll do:
- Welcome and check in families
- Organize teams and help manage Picture Day flow
- Review order forms for completeness and accuracy
- Help keep portrait and team sessions on schedule
- Support photographers in the field

What we're looking for:
- Friendly, professional communication
- Reliable transportation and valid driver's license
- Comfortable standing, working outdoors, and lifting up to 25 pounds
- Ability to pass required background checks

Apply through NESP:
https://careers.nesportsphoto.com/careers/?p=applyToJob&ID=41005

Applications are reviewed by the New England Sports Photo hiring team.
```

## College Career Boards

Use the matching direct application link for each role. Do not create accounts
or publish without Craig's approval.

| Target | Priority | Likely roles | Cost / access |
| --- | --- | --- | --- |
| Northern Essex Community College | High | Customer Service, Field Assistant, Staff Photographer | Employer account likely required; likely free/approval required |
| Merrimack College | High | Staff Photographer, Field Assistant, Customer Service | Employer account likely required; approval required |
| UMass Lowell | High | Staff Photographer, Field Assistant, Customer Service | Employer account likely required; approval required |
| Middlesex Community College | High | Customer Service, Field Assistant, Staff Photographer | Employer account likely required; likely free/approval required |
| Salem State University | Medium | Staff Photographer, Field Assistant | Employer account likely required |
| North Shore Community College | Medium | Customer Service, Field Assistant | Employer account likely required |
| Massachusetts College of Art and Design | Medium | Staff Photographer, Freelance Photographer | Employer account likely required |
| Rhode Island School of Design | Medium | Freelance Photographer, Staff Photographer | Employer account likely required |
| Community College of Rhode Island | Medium | Field Assistant, Staff Photographer | Employer account likely required |
| Southern New Hampshire University | Medium | Customer Service, Staff Photographer, Field Assistant | Employer account likely required |
| University of New Hampshire | Medium | Staff Photographer, Field Assistant | Employer account likely required |
| NHTI - Concord's Community College | Medium | Field Assistant, Customer Service | Employer account likely required |

Suggested categories: `Part-time`, `Seasonal`, `Customer Service`,
`Event Staff`, `Photography`, `Sports`, `Communications`, `Arts`.

## Local Job Boards And Workforce Channels

Do not publish without Craig approval.

| Channel | Suggested use | Cost / access |
| --- | --- | --- |
| MassHire Merrimack Valley Career Center | Customer Service, Field Assistant, Staff Photographer | Likely free; employer account or staff approval required |
| MassHire North Shore Career Center | Field Assistant and Staff Photographer | Likely free; approval required |
| MassHire Lowell Career Center | Customer Service and seasonal field roles | Likely free; approval required |
| New Hampshire Works | Staff Photographer, Freelance Photographer, Field Assistant | Likely free; employer account required |
| EmployRI | Staff Photographer, Freelance Photographer, Field Assistant | Likely free; employer account required |
| CT Hires | Staff Photographer and Freelance Photographer | Likely free; employer account required |
| Local high-school or vocational career offices | Field Assistant, Customer Service | Approval required; use only where employer postings are allowed |

## Photography Communities

Do not publish without Craig approval. Use the freelance role only where
contractor opportunities are allowed and the post can state equipment
requirements clearly.

| Channel | Suggested use | Cost / access |
| --- | --- | --- |
| Professional Photographers Association of Massachusetts | Freelance Photographer, Staff Photographer | Membership or approval may be required |
| Professional Photographers of America job/community boards | Freelance Photographer | Account/membership may be required |
| Local camera clubs in MA, RI, NH, CT | Freelance Photographer, Staff Photographer | Usually free or approval required |
| College photography program newsletters | Staff Photographer, Freelance Photographer | School approval required |
| Local arts/creative job boards | Staff Photographer, Freelance Photographer | Varies |

## General Social Sharing

Use for email, text, LinkedIn personal post, website announcement, or other
non-Facebook sharing Craig approves.

```text
New England Sports Photo is hiring for seasonal youth-sports Picture Day and office support roles.

Open roles:
- Part-Time Customer Service Representative - Methuen office, $22-$25/hour
- Weekend Staff Portrait & Team Photographer - youth sports, $22-$25/hour, NESP equipment provided
- Freelance/Contract Youth Sports Photographer - $22-$27/hour, photographer-provided approved equipment required
- Weekend Table Greeter / Field Assistant - Picture Day field support, $18/hour

NESP is family-owned and has served youth sports organizations and families throughout New England for more than 50 years.

Apply through the NESP careers portal:
https://careers.nesportsphoto.com/careers/?p=showAll

Applications are reviewed by the New England Sports Photo hiring team.
```

## Publication Approval Checklist

Before any external post goes live, Craig should confirm:

- Platform and account are correct.
- Role, pay, territory, schedule, classification, equipment, and application
  link are correct.
- Staff photographer and freelance photographer are clearly separate.
- Contractor posting does not describe employee benefits as contractor benefits.
- No applicant email-resume request is added.
- No unsupported claims such as `#1` or `premier` are used.
- No local-review or draft wording is present.
- No paid sponsorship, boost, billing, CAPTCHA, identity verification, or final
  publish step is pending without Craig present.
- No automatic applicant ranking, rejection, hiring, status changes, or email is
  enabled.
